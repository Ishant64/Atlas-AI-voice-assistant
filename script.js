...
const COMMANDS = {
  ...
  gps: {
    patterns: [/navigate to (.+)|going to (.+)/i],
    response: (match) => {
      const destination = match[1] || match[2]; // Capture the destination from the command
      return `Navigating to ${destination}. Here is the link: https://www.google.com/maps/search?q=${encodeURIComponent(
        destination
      )}`;
    },
  },
